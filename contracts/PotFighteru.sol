...
contract PotFighter is Ownable {
    ...
    function createPot(uint256 _time) external payable userBlackListed {
        ...
        uint256 creationCharges = pot.participationFeee == 0 ? 2000000000000000 : 0;
        pot.participationFeee = creationCharges;
        ...
        userBlackListed
    {
        ...
        require(msg.value == pot.participationFeee, "Pay the fee to join the pot");
    ...
    function getPlayerEarnings(uint256 _potId) external view returns (uint256[6] memory) {
    Pot storage pot = createdPots[_potId];
    uint256 currentSize = pot.participants.length;
    ...
    function getPotInfo(uint256 _potId) external view returns (
    uint256 players,
    uint256 startTime,
    uint256 endTime
) {
    ...
    startTime = pot.participants[0].startedAt; // Starting time of the first player
 ...
 //created pots
 function getMyPots() external view returns (Pot[] memory) {
    uint256 myPotCount = 0;

    // Determine the number of pots created by the caller
    for (uint256 i = 1; i < potId; i++) {
        if (createdPots[i].creator == msg.sender) {
            myPotCount++;
        }
    }

    // Create an array to store the pots created by the caller
    Pot[] memory myPots = new Pot[](myPotCount);
    uint256 currentIndex = 0;

    // Populate the myPots array with the pots created by the caller
    for (uint256 i = 1; i < potId; i++) {
        if (createdPots[i].creator == msg.sender) {
            myPots[currentIndex] = createdPots[i];
            currentIndex++;
        }
    }
 ...
 //JOINED POTS
  ...
  function getPotsForUser(address userAddress) external view returns (uint256[] memory, uint256[] memory, uint256[] memory, uint256[] memory) {
    uint256[] memory potIds;
    uint256[] memory startingTimes;
    uint256[] memory lifeTimes;
    uint256[] memory playerCounts;

    for (uint256 i = 1; i < potId; i++) {
        Pot storage pot = createdPots[i];

        for (uint256 j = 0; j < pot.participants.length; j++) {
            if (pot.participants[j].userAddress == userAddress) {
                potIds = pushValue(potIds, i);
                startingTimes = pushValue(startingTimes, pot.begining);
                lifeTimes = pushValue(lifeTimes, pot.lifeTime);
                playerCounts = pushValue(playerCounts, pot.participants.length);
                
            }
        }
    }

    return (potIds, startingTimes, lifeTimes, playerCounts);
}

function pushValue(uint256[] memory array, uint256 value) internal pure returns (uint256[] memory) {
