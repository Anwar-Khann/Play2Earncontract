...
contract Pot22 is Ownable {
    ...
    function createPot() external payable userBlackListed {
        ...
        pot.participants.push(
            participant({
                ...
                startedAt: block.timestamp, // THE CURRENT TIME AT WHICH GAME IS STARTED
        ...
        
    {
        ...
        //getting length of current pot player
        uint256 currentSize = pot.participants.length;
        participant storage p = pot.participants[currentSize - 1];
        //updating the time and status etc of last player;
        p.endedAt = block.timestamp;
        p.durationPlayed = (p.endedAt - p.startedAt);
        uint256 durationReference = p.durationPlayed;
        //now count reward total
        bool rewardMonitor;
        for (uint256 i = 0; i <= durationReference; i++) {
